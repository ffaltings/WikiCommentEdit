...
def get_cc_url(index_file, task_index):
    lines = list(read_lines(index_file))
    try: return lines[task_index]
    except: return None
...
if __name__ == "__main__":
    ...
    parser.add_argument('--temp-path', type=str, default="./data/raw/cc-index/", help='the temp / data directory, used to download wiki dump files')
    ...
    logging.info("Determining index joining task..")
    index_url = get_cc_url(args.cc_index_file, args.index)
    logging.info("Working on index url: " + index_url)
    ...
    process(
        cc_index_input_stream,
        base_generator = iterate_lines, # chose base generator here
